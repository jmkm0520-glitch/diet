...
def import_rows(client, request: ImportRequest) -> dict[str, int]:
    weights = [{"date": row.date.isoformat(), "weight": row.weight} for row in request.rows if row.weight is not None]
    meals = [{"date": row.date.isoformat(), **meal.model_dump(mode="json")} for row in request.rows for meal in row.meals]
    if weights:
        client.table("weights").upsert(weights, on_conflict="date").execute()
    if meals:
        client.table("meals").upsert(meals, on_conflict="date,meal").execute()
    return {"days": len(request.rows), "weights": len(weights), "meals": len(meals)}

class handler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:
        try:
            request = ImportRequest.model_validate(json.loads(read_json_body(self, max_bytes=512 * 1024)))
        except (RequestBodyTooLargeError, UnsupportedMediaTypeError, json.JSONDecodeError, ValidationError, ValueError):
            _send_json(self, status_for_error_code("VALIDATION_ERROR"), validation_error_response())
            return
        try:
            _send_json(self, 200, success_response(import_rows(get_supabase_client(), request)))
