...
이 앱은 Supabase Auth의 단일 회원 계정으로 보호합니다. 브라우저는 HttpOnly 세션 쿠키를 사용하고,
Python API가 토큰과 `members` 프로필을 검증합니다. 식단과 체중 쿼리는 모두 `member_id` 조건을
포함합니다. 최초 회원 생성 주소는 공개 가입 기능이 아니며, 한 명이 생성된 뒤 DB 함수가 추가
생성을 거부합니다.

현재 결정:

1. 최초 회원 생성 전에는 Vercel Deployment Protection 같은 배포 접근 제한을 사용한다.
2. 신뢰할 수 있는 관리자만 최초 회원을 생성한다.
3. 공개 전 미인증 API의 401 응답과 기존 데이터의 회원 연결 상태를 확인한다.
4. 공개 전 `pnpm security:check`와 전체 테스트를 다시 실행한다.

상세 설계와 배포 순서는 [단일 회원 인증 설계](./MEMBER_AUTH_DESIGN.md)를 참고한다.
